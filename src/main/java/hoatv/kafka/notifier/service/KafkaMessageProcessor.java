...
public class KafkaMessageProcessor {
    ...
    private void processConfigurationForMessage(NotifierConfiguration config,
        ...
        try {
            ...
            if (rulesMatch) {
                LOGGER.info("Rules matched for configuration: {} on topic: {}. Executing actions.", config.getNotifier(), topic);
                for (NotificationAction action : config.getActions()) {
                    executeAction(action, message, config);
                }
            } else {
                LOGGER.debug("Rules did not match for configuration: {} on topic: {}", config.getNotifier(), topic);
            }
